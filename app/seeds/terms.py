...
def seed_terms(db: Session) -> TermsTableModel:
    ...
    # Check if version v1.0 already exists
    existing_terms = db.query(TermsTableModel).filter_by(version="v1.0").first()
    if existing_terms:
        print(f"* Terms v1.0 already exists ({existing_terms.id})")
        return existing_terms

    print("Seeding Terms v1.0...")

    terms = TermsTableModel(
        version="v1.0",
        content=TERMS_CONTENT_V1,
    ...
    print(f"* Created Terms v1.0 with ID: {terms.id}")
    return terms
