...
class ItemService:
    ...
    def create_item(self, user: User, item_in: ItemCreate) -> ItemRead:
        ...
        if item_in.file_ids:
            self.file_service.create_associations(
                file_ids=item_in.file_ids, entity_id=created_item.id, entity_type="item", category="gallery"
            )
    ...
    def update_item(self, item_id: uuid.UUID, user: User, item_in: ItemUpdate) -> ItemRead:
        ...
        if item_in.file_ids is not None:
            self.file_service.delete_associations_for_entity(item.id, "item")
            self.file_service.create_associations(item_in.file_ids, item.id, "item", "gallery")
