...
def seller_token(client):
    ...
    response = client.post("/auth/login", data={"username": "jane_smith", "password": settings.DEFAULT_PASSWORD})
    return response.json()["access_token"]
...
def user_token(client):
    ...
    response = client.post("/auth/login", data={"username": "basic_user", "password": settings.DEFAULT_PASSWORD})
    return response.json()["access_token"]
...
def user2_token(client):
    ...
    response = client.post("/auth/login", data={"username": "carlos_mendez", "password": settings.DEFAULT_PASSWORD})
    return response.json()["access_token"]
...
@pytest.fixture
def seller_user(db):
    """Get 'jane_smith' (seller) user object from DB."""
    return db.query(User).filter_by(username="jane_smith").first()


@pytest.fixture
def basic_user(db):
    """Get 'basic_user' object from DB."""
    return db.query(User).filter_by(username="basic_user").first()


@pytest.fixture
def basic_user_2(db):
    """Get 'jane_smith' user object from DB (for test_channels)."""
    return db.query(User).filter_by(username="jane_smith").first()
