...
class TestTermsEndpoints:
    ...
    def test_get_terms_by_id(self, client, auth_headers, admin_auth_headers):
        ...
        resp = client.get(f"/terms/{terms_id}", headers=auth_headers)
        assert resp.status_code == 200
    ...
    def test_get_terms_by_id_not_found(self, client, auth_headers):
        ...
        resp = client.get(f"/terms/{random_id}", headers=auth_headers)
    ...
    def test_get_terms_by_version(self, client, auth_headers, admin_auth_headers):
        ...
        client.post("/terms/", json=payload, headers=admin_auth_headers)

        resp = client.get(f"/terms/version/{target_version}", headers=auth_headers)
    ...
    def test_get_terms_by_version_not_found(self, client, auth_headers):
        resp = client.get("/terms/version/v.NON_EXISTENT", headers=auth_headers)
        assert resp.status_code == 404
    ...
    def test_update_terms_as_admin(self, client, admin_auth_headers):
        ...
        create_resp = client.post("/terms/", json=payload, headers=admin_auth_headers)
        terms_id = create_resp.json()["id"]

        update_payload = {"content": "Updated content by Admin."}

        resp = client.patch(f"/terms/{terms_id}", json=update_payload, headers=admin_auth_headers)
    ...
    def test_update_terms_as_basic_user(self, client, auth_headers, admin_auth_headers):
        ...
        resp = client.patch(f"/terms/{terms_id}", json=update_payload, headers=auth_headers)

        assert resp.status_code == 403

    def test_update_terms_not_found(self, client, admin_auth_headers):
        random_id = uuid.uuid4()
        resp = client.patch(f"/terms/{random_id}", json={"content": "New"}, headers=admin_auth_headers)
        assert resp.status_code == 404
    ...
    def test_delete_terms_as_admin(self, client, admin_auth_headers, db):
        ...
        resp = client.delete(f"/terms/{terms_id}", headers=admin_auth_headers)
        assert resp.status_code == 204

        get_resp = client.get(f"/terms/{terms_id}", headers=admin_auth_headers)
    ...
    def test_delete_terms_as_basic_user(self, client, auth_headers, admin_auth_headers):
        ...
        resp = client.delete(f"/terms/{terms_id}", headers=auth_headers)
        assert resp.status_code == 403

    def test_delete_terms_not_found(self, client, admin_auth_headers):
        random_id = uuid.uuid4()
        resp = client.delete(f"/terms/{random_id}", headers=admin_auth_headers)
        assert resp.status_code == 404
